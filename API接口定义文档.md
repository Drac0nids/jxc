...
| 文档属性 | 内容 |
| --- | --- |
| 版本 | v1.8.0（新增：Android 单机版（进程内嵌）接口与鉴权兼容约定，见 21） |
...
> **实现状态（2026-09 修订）**：`4091` 尚未生效。`expected_version` 已按本文档在请求体中
> 预留并由路由透传，但仓储层还没有消费它，因此当前不会因版本不一致返回 `4091`。
> 本文档中所有「失败返回 `4091`」的描述均以此实现状态为准；待仓储层落地后无需再改本文档。
...
## 20. 单机版（本地模式）接口与鉴权兼容约定（v1.7.0 新增）

### 20.1 接口兼容性

- 单机版复用全部既有接口路径、请求结构与响应结构，**不新增接口、不改变错误码**。
- 差异只在存储后端：`STORAGE_BACKEND=sqlite` 时数据落在本地 SQLite 数据文件；事务、库存并发、幂等与乐观锁冲突码语义与 PostgreSQL 版一致。

### 20.2 服务地址与配置

| 项 | 说明 |
| --- | --- |
| 默认地址 | `http://127.0.0.1:8080/api/v1`（内嵌服务端由桌面壳拉起） |
| `STORAGE_BACKEND` | `postgres`（默认）/ `memory` / `sqlite` |
| `SQLITE_PATH` | SQLite 数据文件路径，默认 `jxc.db` |

### 20.3 单机模式租户与鉴权约定

1. 单机版为单租户形态：首次启动自动创建租户（租户码 `local`）与初始管理员账号。
2. 登录请求体中的租户码在单机模式下可选：未提供时默认使用 `local`；PostgreSQL 多租户模式下租户码仍为必填。
3. 单机模式不提供跨租户能力：`tenant_id` 固定为本地租户，租户隔离语义保持不变。
...
- `X-Idempotency-Key` 与 `expected_version` 在单机模式下同样强制生效。
- 重放与冲突行为不变：`4090`（业务冲突）、`4091`（版本冲突）、`4092`（幂等请求体冲突）。

### 20.5 兼容约束

1. 既有 SaaS 客户端不受影响：未显式设置 `STORAGE_BACKEND` 时默认 `postgres`。
2. 单机版与 SaaS 版使用同一份前端页面，差异仅在默认服务地址与是否内嵌服务端。
3. 新增字段或约束时必须同时更新 PostgreSQL 与 SQLite 两份迁移脚本，接口契约以本文档为准。

---

## 21. Android 单机版（进程内嵌）接口与鉴权兼容约定（v1.8.0 新增）

### 21.1 接口兼容性

- 单机版复用全部既有接口路径、请求结构与响应结构，**不新增接口、不改变错误码**。
- 服务端在 App 进程内运行，仍通过 HTTP 暴露同一套 API；客户端代码无需区分形态。

### 21.2 服务地址与配置

| 项 | 说明 |
| --- | --- |
| 默认地址 | `http://127.0.0.1:<port>/api/v1`，端口由系统分配（不是固定 8080） |
| 生效方式 | App 启动时调用内嵌服务端，拿到端口后覆盖全局 API 地址 |
| 数据文件 | 应用私有目录下 `jxc.db`（SQLite） |
| 存储后端 | 强制 `sqlite`（单机形态），不依赖 PostgreSQL / Redis |

### 21.3 单机模式鉴权约定

1. 单机版为单租户形态：首次启动自动创建租户（租户码 `local`）与管理员账号（`admin` / `admin123`）。
2. 客户端在单机模式下**固定使用租户码 `local`** 并隐藏注册入口；`POST /auth/register` 在单机版不对外暴露为可用入口。
3. 登录请求体中的租户码在 `sqlite` 后端下可选（服务端回落 `local`）；`postgres` 多租户模式下仍为必填。
4. `tenant_id` 固定为本地租户，租户隔离语义不变。

### 21.4 幂等与冲突

- `X-Idempotency-Key` 与 `expected_version` 在单机模式下同样强制生效。
- 冲突码语义不变：`4090`（业务冲突）、`4091`（版本冲突）、`4092`（幂等请求体冲突）。

### 21.5 兼容约束

1. 内嵌服务端不可用（非 Android 平台或加载失败）时，客户端回退为 `--dart-define=API_BASE_URL` 指定的远端服务，行为与既有 SaaS 客户端一致。
2. 单机版与 SaaS 版共用同一份接口契约与前端页面，差异仅在服务地址、租户码处理与是否暴露注册入口。
3. 三种形态（SaaS / Windows 单机 / Android 单机）的接口契约以本文档为唯一准绳。
